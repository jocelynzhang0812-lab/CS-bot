...
from csbot.nlp.aliases import expand_for_intent_detection, expand_for_kb_search
...
class KBSearchSkill(BaseTool):
    ...
    SOURCE_PRODUCT_MAP = {
        # Help center — 按产品隔离
        # 注：kimi-code.md 设为 general，因为 claw 用户大量询问 coding plan / bridge 额度问题
        "help_center/kimi-code.md": "general",
        ...
        "kimi claw分类/问题上报与人工客服处理.md": "claw",
    ...
    PRODUCT_KEYWORDS = {
        ...
        "app": {
            ...
            "kimi.com", "对话", "聊天",
            "会员", "订阅", "充值", "付费", "套餐",
            "allegretto", "moderato", "andante", "allegro", "adagio",
        },
        "research": {
    ...
    # 用于判断查询是否与会员相关的上下文词
    MEMBERSHIP_CONTEXT_KEYWORDS = {
        "会员", "套餐", "价格", "多少钱", "费用", "收费", "元", "¥", "￥",
        "订阅", "充值", "付费", "升级", "降级", "购买", "开通", "续费",
        "andante", "moderato", "allegretto", "allegro", "adagio",
    }

    # ── 相似度阈值：低于此值的 chunk 视为不相关，直接过滤 ──
    # 2025-05-09 优化：从 0.45 降到 0.35，避免关键词-only 模式下大量有效结果被误杀
    ...
    def _detect_product(self, query: str) -> str:
        ...
        if not scores or max(scores.values()) == 0:
            # 未匹配到任何产品关键词时，检查是否为常见客服/故障场景，默认归为 claw
            q_lower = query.lower()
            fallback_keywords = {
                "错误", "bug", "崩溃", "离线", "退款", "额度",
                "客服", "人工", "失败", "连接不上", "没有反应", "无法",
                "不能", "报错", "异常", "卡住", "不动", "重启", "失效", "过期",
                "不能用", "打不开", "登不上", "连不上", "没反应",
                "会员", "订阅", "充值", "付费", "套餐",
            }
            if any(kw in q_lower for kw in fallback_keywords):
                return "claw"
            return "uncertain"
        ...
        for prod, sc in scores.items():
            if prod != best and sc > 0 and (best_score - sc) < self.UNCERTAINTY_GAP:
                # 差距模糊时，同样 fallback 到常见客服词判断
                q_lower = query.lower()
                fallback_keywords = {
                    "错误", "bug", "崩溃", "离线", "怎么办", "怎么", "退款", "额度",
                    "客服", "人工", "问题", "失败", "连接不上", "没有反应", "无法",
                    "不能", "报错", "异常", "卡住", "不动", "重启", "失效", "过期",
                    "不能用", "打不开", "登不上", "连不上", "没反应",
                    "会员", "订阅", "充值", "付费", "套餐",
                }
    ...
    @staticmethod
    def _extract_price_numbers(text: str) -> set:
        """从文本中提取价格数字。支持 ¥123、123元、123/月、123个月，
        以及独立的2-3位数字（排除明显年份 1900-2099）。"""
        prices = set()
        # 明确的价格表达：¥123 或 123元/123/月/123个月
        for m in re.finditer(r'[¥￥](\d+)|(\d+)(?:元|/月|/年|个月|月)', text):
            prices.add(m.group(1) or m.group(2))
        # 独立的2-3位数字
        for m in re.finditer(r'(?<![\d.])\d{2,3}(?![\d.])', text):
            val = m.group(0)
            if not (1900 <= int(val) <= 2099):
                prices.add(val)
        return prices

    def _validate_membership_price(self, query: str) -> Dict[str, Any]:
        """校验会员价格查询。

        返回结构:
        {
            "is_membership_query": bool,
            "has_price": bool,
            "prices": List[str],
        ...
        q = query.lower()
        # 判断是否涉及会员/套餐/价格上下文
        is_membership = any(k in q for k in self.MEMBERSHIP_CONTEXT_KEYWORDS)
        if not is_membership:
            return {"is_membership_query": False, "is_valid": True, "reason": ""}

        prices = self._extract_price_numbers(query)
        if not prices:
            return {"is_membership_query": True, "has_price": False, "is_valid": True, "reason": ""}

        invalid = [p for p in prices if p not in self.MEMBERSHIP_PRICE_MAP]
        if invalid:
            price_str = "、".join(f"¥{p}" for p in sorted(invalid, key=int))
            valid_prices = "、".join(
                f"¥{k}({v})" for k, v in sorted(self.MEMBERSHIP_PRICE_MAP.items(), key=lambda x: int(x[0]))
            )
            return {
                "is_membership_query": True,
                "has_price": True,
                "prices": sorted(list(prices), key=int),
                "invalid_prices": sorted(invalid, key=int),
                "is_valid": False,
                "reason": (
                    f"用户提到的 {price_str} 不是 Kimi 官方会员套餐价格。"
                    f"目前官方会员套餐为：{valid_prices}。"
                ),
            }

        return {"is_membership_query": True, "has_price": True, "is_valid": True, "reason": ""}

    async def execute(self, query: str, top_k: int = 3, **kwargs) -> ToolResult:
        # ── 会员价格硬校验：非标准价格直接拒绝 ──
        price_validation = self._validate_membership_price(query)
        if price_validation["is_membership_query"] and not price_validation["is_valid"]:
            return ToolResult(
                tool_name=self.name,
        ...
        # 为知识库检索扩展查询（同义词追加，提升召回率）
        search_query = expand_for_kb_search(query)
        ...
        results = await self.index.search(search_query, top_k=top_k * 3, provider=self.provider)
        filtered = self._filter_by_product(results, product)

        # ── 相似度阈值过滤：低于阈值的 chunk 视为不相关，直接剔除 ──
        filtered = [r for r in filtered if r.score >= self.SIMILARITY_THRESHOLD]
        ...
        # ── Debug 日志：打印召回结果，方便检查 chunk 质量 ──
        print(f"[KBSearch] query='{query}' product={product} threshold={self.SIMILARITY_THRESHOLD}")
        if hits:
            for h in hits:
                print(f"  [chunk {h['id']}] score={h['score']} source={h['source']} title={h['title'][:40]}")
        else:
            # 打印被过滤掉的结果，帮助调优阈值
            print(f"  [WARN] 无命中结果。原始召回 top3:")
            for i, r in enumerate(results[:3], 1):
                print(f"    [raw {i}] score={round(r.score, 3)} source={r.doc.source} title={r.doc.title[:40]}")

        # 只有当所有 chunk 分数都 >= 阈值时才算 hit
        hit = len(hits) > 0
        ...
        return ToolResult(
            ...
            result={
                ...
                "reply_hint": "基于知识库直接回答" if hit else "未命中，进入排查流程",
            },
        )
