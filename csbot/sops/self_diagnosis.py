            ...
            if any(k in sym for k in r["kw"]):
                ...
                return ToolResult(
                    ...
                    result={
                        "is_known_issue": True,
                        "rule_id": r["id"],
                        # root_cause 和 workaround 不再返回给 LLM，防止被直接转述
                        "root_cause": "该问题已被记录，具体原因需由技术团队排查确认。",
                        "workaround": "请联系人工客服获取进一步协助。",
                        "needs_feedback_bot": False,
                        # user_reply 不再包含具体修复步骤
                        "user_reply": "该问题已被记录，建议您联系人工客服进一步协助。",
                    },
                )
        return ToolResult(
            tool_name=self.name,
