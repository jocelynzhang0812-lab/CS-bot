...
class CSOutputReviewerSkill(BaseTool):
    ...
    def __init__(self):
        ...
        self.forbidden_confidence_when_kb_miss = [
            ...
            "你可以尝试", "您可以尝试", "建议你", "建议您",
            "可能的原因是", "可能是因为", "修复方法", "修复步骤",
            "修改代码", "更新配置", "检查一下", "请检查",
            "试试看", "试一下", "换个方式", "另一种方法",
            "代码如下", "配置如下", "步骤如下",
            "命令如下", "执行以下", "运行以下", "请执行", "请运行",
            "输入以下", "在终端", "在命令行", "修改配置文件",
            "添加配置", "删除配置", "重启服务", "重新部署",
            "升级版本", "降级版本", "配置示例",
        ]
    ...
    async def execute(
        ...
        **kwargs,
        ...
        # ── 10.5 包含未格式化的代码/命令/路径（kb_hit=True 时也要检查）──
        # 检测看起来像命令、文件路径、JSON 配置的内容
        unformatted_code_signals = [
            r"~\s*/\.\w+/\w+\.\w+",           # ~/.openclaw/openclaw.json
            r"/(?:usr|bin|opt|home|root)/[\w/]+",  # /usr/local/bin/xxx
            r"(?:^|\n)\s*(?:cp|mv|rm|cat|ls|cd|mkdir|chmod|openclaw|npx|npm|pip|python|curl|wget|git|docker)\s+\S+",
            r"\{\s*\"\w+\"\s*:\s*\{",        # JSON 对象开头
            r"--[\w\-]+",                       # 命令行选项 --fix
        ]
        found_unformatted = []
        for pat in unformatted_code_signals:
            if re.search(pat, checked_reply, re.IGNORECASE | re.MULTILINE):
                found_unformatted.append(pat)
        if found_unformatted:
            # 知识库未命中时，任何未格式化代码都是高危
            # 知识库命中时，也要告警（由 CodeGroundingChecker 兜底，这里是双保险）
            severity = "high" if not kb_hit else "medium"
            violations.append({
                "rule": "unformatted_code_or_command",
                "severity": severity,
                "detail": f"回复中包含未格式化的代码/命令/路径信号：{found_unformatted[:2]}。",
                "fix": "严禁基于训练数据生成未经验证的命令、文件路径或配置片段。",
            })
            if not kb_hit:
                checked_reply = "抱歉，这个问题我暂时无法回答，建议您联系人工客服进一步协助。"
