...
class KnowledgeIndex:
    ...
    def add(self, doc: KnowledgeDoc):
        ...
        # 关键词索引
        all_kw = doc.keywords + doc.title.split() + doc.tags
        for kw in all_kw:
            kw_lower = kw.lower()
            self._keyword_map.setdefault(kw_lower, []).append(doc.id)
    ...
    def _keyword_search_scores(self, query_lower: str) -> Dict[str, float]:
        ...
        for did, doc in self._docs.items():
            if any(w in doc.title.lower() for w in q_words):
                scores[did] = scores.get(did, 0) + 0.3
